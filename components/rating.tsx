...
function Circle(value: number, index: number) {
  return (
    <span className="circle-container" aria-hidden="true" key={index}>
      {value == 1 && <span data-testid="full-circle" className="full-circle"></span>}
      {(value == 0.25 || value == 0.75) && (
        <span data-testid="quarter-circle" className="quarter-circle"></span>
      )}
      {(value == 0.5 || value == 0.75) && (
        <span data-testid="half-circle" className="half-circle"></span>
      )}
    </span>
  )
}
